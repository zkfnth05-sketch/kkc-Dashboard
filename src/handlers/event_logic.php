...
function kkc_handle_event_list($input) {
    ...
    foreach ($tables_to_query as $table) {
        ...
        // 🚀 Fetch more to allow PHP filtering, but without excessive load
        $results = $wpdb->get_results("SELECT * FROM $table WHERE $where ORDER BY ds_date DESC LIMIT 1000", ARRAY_A);
        foreach ($results as $row) {
            ...
            $count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $app_table WHERE ds_pid = %d", $row['ds_pid']));
