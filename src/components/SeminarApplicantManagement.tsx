...
import { BRIDGE_URL, SECRET_KEY, fetchMembers } from '../services/memberService';

interface SeminarApplicant {
    id: string;
...
interface SeminarApplicantManagementProps {
...
export const SeminarApplicantManagement: React.FC<SeminarApplicantManagementProps> = ({
    ...
    const loadData = async () => {
        ...
        setIsLoading(true);
        try {
            const res = await fetchApplicants(parsedDsPid, 'seminar_applicant');
            if (res.data) {
    ...
    const handleOpenModal = (applicant?: SeminarApplicant) => {
        if (applicant) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            let res;
            if (editTargetId) {
                res = await updateApplicant({ ...payload, id: editTargetId }, 'seminar_applicant');
            } else {
                res = await createApplicant(payload, 'seminar_applicant');
            }
    ...
    const handleDelete = (id: string, name: string) => {
        showConfirm('신청자 삭제', `'${name}' 신청자를 삭제하시겠습니까?`, async () => {
            setIsLoading(true);
            try {
                const res = await deleteApplicant(id, 'seminar_applicant');
                if (res.success) {
                ...
                <table className="w-full text-[13px] border-collapse text-center">
                    <thead>
                        <tr className="bg-gray-50 border-b border-gray-200 text-gray-500 font-bold sticky top-0 z-10">
                            ...
                            <th className="py-4 px-2 w-[18%] font-black border-r border-gray-100 text-left px-4">이메일</th>
                            <th className="py-4 px-2 w-[20%] font-black border-r border-gray-100">소속 (직업)</th>
                            <th className="py-4 px-2 w-[10%] font-black border-r border-gray-100">입금 상태</th>
                            <th className="py-4 px-3 w-[15%] font-black">관리</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                        {applicants.length > 0 ? applicants.map((item) => (
                            <tr key={item.id} className="hover:bg-gray-50/80 transition-colors group">
                                ...
                                <td className="py-4 px-4 border-r border-gray-50 text-left text-gray-600 font-medium">{item.email || '-'}</td>
                                <td className="py-4 px-2 border-r border-gray-50 text-gray-600">{item.affiliation || '-'}</td>
                                ...
                                <td className="py-4 px-3">
                                    <div className="flex justify-center gap-1.5">
                                        <button onClick={() => setSelectedOptionsApp(item)} className="px-3 py-1.5 bg-blue-50 border border-blue-100 rounded text-[11px] font-bold text-blue-600 hover:bg-blue-100 transition-all">옵션보기</button>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[95vh]">
                        <div className="flex justify-between items-center px-10 py-8 bg-gray-50/50 border-b border-gray-100">
                            <h3 className="text-[22px] font-black text-gray-900 tracking-tight">{editTargetId ? '세미나 정보 수정' : '세미나 신청 등록'}</h3>
                        ...
                        <div className="p-10 overflow-y-auto space-y-8">
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-1.5">
                                    <label className="text-[13px] font-black text-gray-500">회원 ID</label>
                                    <div className="flex gap-2">
                                        <input
                                            type="text"
                            ...
                            <div className="grid grid-cols-3 gap-6">
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[13px] font-black text-gray-500">생년월일</label>
                                    <input
                                        type="text"
                                        className="w-full border border-gray-300 rounded-xl px-4 py-3 text-[14px] font-bold shadow-inner text-center"
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[13px] font-black text-gray-500">이메일</label>
                                    <input
                                        type="email"
                                        className="w-full border border-gray-300 rounded-xl px-4 py-3 text-[14px] font-bold shadow-inner"
                            ...
                            <div className="space-y-1.5">
                                <label className="text-[13px] font-black text-gray-500">소속 (직업)</label>
                                <input
                                    type="text"
                                    className="w-full border border-gray-300 rounded-xl px-4 py-3 text-[14px] font-bold shadow-inner"
                                    value={formData.affiliation}
                                    onChange={e => setFormData({ ...formData, affiliation: e.target.value })}
                                    placeholder="소속기관 또는 직업 입력"
                                />
                            </div>
