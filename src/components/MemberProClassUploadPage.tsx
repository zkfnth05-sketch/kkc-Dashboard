...
export const MemberProClassUploadPage: React.FC<MemberProClassUploadPageProps> = ({ onClose, onUpload }) => {
    ...
    const handleDownloadTemplate = () => {
        const headers = [
            '아이디', '이름', '생년월일(YYMMDD)', '가입일(YYYY-MM-DD)', '핸드폰', '주소', '이메일', '등급(예:B0)', '자격내용(메모)'
        ];
        // Add dummy row
        const dummyrow = [
            'user_01', '홍길동', '800101', '2023-01-01', '010-1234-5678', '서울시 강남구', 'hong@test.com', 'B0', '2023년 취득'
        ];
        downloadCsv(headers.join(',') + '\n' + dummyrow.join(','), '직능업데이트_템플릿.csv');
    };
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!isSkillDisabled && !selectedSkill) {
            setErrorMsg("먼저 추가할 직능을 선택하거나 '직능 선택 안함'을 체크해주세요.");
            setSelectedFileName(null);
            ...
            complete: (results) => {
                try {
                    const members = results.data.map((row: any) => {
                        ...
                        const name = row['이름']?.trim() || row['성명(한글)']?.trim() || row['성명']?.trim();
                        ...
                        if (!name) throw new Error(`${loginId} 회원의 이름이 누락되었습니다.`);
                        if (!birth) throw new Error(`${loginId} 회원의 생년월일이 누락되었습니다.`);
                        ...
                        return {
                            ...
                            addr: row['주소'] || '',
                            email: row['이메일'] || '',
                            rank: row['등급(예:B0)'] || row['등급'] || '',
                            memo: row['자격내용(메모)'] || row['자격내용'] || row['메모'] || ''
                        };
                    });
            ...
            <div className="flex-1 overflow-y-auto p-10">
                <div className="max-w-[800px] mx-auto">
                    <div className="flex justify-between items-center mb-8">
                        <h2 className="text-2xl font-bold text-gray-800">엑셀로 직능 및 정보 일괄 업데이트</h2>
                        <button
                            onClick={onClose}
                    ...
                    <div className="bg-white rounded border border-gray-200 shadow-sm p-8 mb-6 relative">
                        <h3 className="text-xl font-bold text-gray-800 mb-4">1. 추가할 직능 선택</h3>
                        ...
                        <div className="mb-8 p-4 bg-gray-50 border border-gray-200 rounded">
                            <label className="flex items-center gap-2 mb-3 cursor-pointer">
                                <input
                                    ...
                                    checked={isSkillDisabled}
                                    onChange={(e) => {
                                        setIsSkillDisabled(e.target.checked);
                                        if (e.target.checked) setSelectedSkill('');
                                    }}
                                />
                                <span className="text-sm font-medium text-gray-700">직능 선택 안함 (직능 추가 없이 회원 정보/메모만 업데이트)</span>
                            </label>
                            <select
                                className="w-full border border-gray-300 rounded p-2.5 outline-none focus:border-blue-500 disabled:bg-gray-100 disabled:text-gray-400"
                        ...
                        <h3 className="text-xl font-bold text-gray-800 mb-2">2. 회원 명단 엑셀 파일 업로드</h3>
                        <p className="text-gray-500 text-sm mb-6">
                            엑셀(.csv) 파일을 업로드하여 회원의 직능과 추가 정보를 대량으로 업데이트할 수 있습니다.
                        </p>
                    ...
                    <div className="bg-gray-100 rounded border border-gray-200 p-8 shadow-inner">
                        ...
                        <ul className="list-disc list-inside text-sm text-gray-600 space-y-2 ml-2 font-medium">
                            ...
                            <li>자격내용(메모) <span className="text-gray-400 font-normal">(선택)</span> - (메모에 추가될 내용)</li>
                        </ul>
