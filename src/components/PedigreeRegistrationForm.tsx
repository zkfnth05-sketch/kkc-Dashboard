...
import { fetchLastRegNo, checkRegNoExists, fetchHairs } from '../services/pedigreeService';
...
export const PedigreeRegistrationForm: React.FC<PedigreeRegistrationFormProps> = ({ initialData, onSave, onCancel }) => {
  ...
  useEffect(() => {
    fetchHairs().then(hairs => setHairOptions(hairs.sort((a, b) => a.localeCompare(b)))).catch(console.error);
  }, []);
    ...
    try {
      ...
      if (dogList.length > 0) {
        ...
        if (dogSex !== expectedSex && dogSex !== (expectedSex === 'M' ? '수컷' : '암컷')) {
          ...
          const info = { name: dog.name, breed: dog.dog_class, sex: dog.sex, uid: dog.uid };
          if (type === 'sire') {
            setSireInfo(info);
            setCommonData(prev => ({ ...prev, sireUid: dog.uid }));
          } else {
            setDamInfo(info);
            setCommonData(prev => ({ ...prev, damUid: dog.uid }));
          }
        }
      } else {
          ...
          <div className="grid grid-cols-12 gap-5">
            ...
            <div className="col-span-12 lg:col-span-4 xl:col-span-3 space-y-5">
              <div className={sectionCardStyle}>
                 ...
                 <div className="border-t border-slate-100 pt-5">
                   <h3 className={subTitleStyle}>기본 정보</h3>
                   <div className="space-y-3">
                     ...
                     <div className="pt-2 flex gap-2 items-end">
                       <div className="flex-1">
                         <label className={labelStyle}>소유자</label>
                       ...
                       <button onClick={() => setCommonData(prev => ({ ...prev, owner: prev.breeder, ownerEng: prev.breederEng, ownerPhone: prev.breederPhone, ownerAddr: prev.breederAddr }))} className="h-9 px-3 bg-[#3b82f6] text-white text-[11px] font-bold rounded-md hover:bg-blue-600 transition-colors">동일</button>
                     </div>
                     <div>
            ...
            <div className="col-span-12 lg:col-span-4 xl:col-span-3 space-y-5">
              <div className={sectionCardStyle}>
                 ...
                 <div className="grid grid-cols-2 gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <input type="text" readOnly className="w-full bg-slate-50 border border-slate-200 rounded-md h-9 px-3 text-[12px] text-slate-400 outline-none" value={commonData.breed} />
                    </div>
                 </div>
                 <div>
              ...
              <div className={sectionCardStyle}>
                 ...
                 {sireInfo ? (
                   <div className="mt-3 p-2 bg-blue-50/50 rounded-lg text-[11px] border border-blue-100 flex items-center gap-2 animate-in slide-in-from-top-1 duration-200">
                   ...
                   <div className="mt-2 text-[10px] text-red-500 font-bold flex items-center gap-1 animate-in shake-in duration-200">
              ...
              <div className={sectionCardStyle}>
                 ...
                 {damInfo ? (
                   <div className="mt-3 p-2 bg-pink-50/50 rounded-lg text-[11px] border border-pink-100 flex items-center gap-2 animate-in slide-in-from-top-1 duration-200">
              ...
              <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-100 flex flex-col h-full min-h-[600px]">
                 <div className="flex justify-between items-center mb-6">
                    <div className="flex items-center gap-4">
                       ...
                       <button 
                         ...
                         className="bg-[#10b981] text-white px-3 py-1.5 rounded-md text-[10px] font-bold hover:bg-emerald-600 transition-colors flex items-center gap-1 disabled:opacity-50"
                       >
                          {isAssigningRegNo ? <Loader2 size={12} className="animate-spin" /> : null}
                          등록번호 자동 부여
                       </button>
                    </div>
                 ...
                 <div className="bg-[#f8fafc] border border-slate-200 rounded-lg p-6 space-y-5 flex-1 relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                          ...
                          <div className="flex gap-2 relative">
                            <input 
                              type="text" 
                              className={`${inputStyle} flex-1 ${searchError.regNo?.[currentDogIndex] ? 'border-red-400 focus:border-red-400' : ''}`}
                      ...
                      <div>
                          ...
                          <select className={inputStyle} value={dogsData[currentDogIndex].color} onChange={e => handleDogChange('color', e.target.value)}>
                              ...
                              {hairOptions.map((h, i) => <option key={i} value={h}>{h}</option>)}
                          </select>
                      </div>
                      <div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <div><label className={labelStyle}>국내 타단체</label><input type="text" className={inputStyle} value={dogsData[currentDogIndex].otherOrg} onChange={e => handleDogChange('otherOrg', e.target.value)} /></div>
                      <div><label className={labelStyle}>외국번호</label><input type="text" className={inputStyle} value={dogsData[currentDogIndex].foreignNo} onChange={e => handleDogChange('foreignNo', e.target.value)} /></div>
                 ...
                 <div className="mt-8 flex justify-end gap-2">
                    ...
                    <button 
                      onClick={() => onSave({ commonData, dogsData })}
