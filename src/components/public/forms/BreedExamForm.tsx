        ...
        >
            <div className="grid grid-cols-1 gap-8">
                ...
                <div className="space-y-2">
                    ...
                    <input name="name" value={formData.name} onChange={handleInputChange} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-teal-500 outline-none transition-all shadow-inner" placeholder="신청자 이름 입력" />
                </div>

                {/* 연락처 */}
                <div className="space-y-2">
                    <label className="text-sm font-bold text-slate-600">연락처</label>
                    <input name="contact" value={formData.contact} onChange={handleInputChange} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-teal-500 outline-none transition-all shadow-inner" placeholder="연락처 (선택)" />
                </div>
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                        <input name="pedigree_number" value={formData.pedigree_number} onChange={handleInputChange} className="flex-1 p-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-teal-500 outline-none transition-all shadow-inner uppercase font-mono" placeholder="등록번호 (선택)" />
                        <button
                            type="button"
                            onClick={handleSearchMember}
                            disabled={isSearching}
                            className="px-6 bg-slate-900 border border-slate-800 text-white rounded-2xl font-bold text-xs hover:bg-black transition-all flex items-center gap-2 active:scale-95"
                        >
